...
namespace parallel_solver
{
	...
	class BICGSTAB
	{
		...
		MPI_Comm mmult_comm_recv;
		MPI_Comm mmult_comm_send;
		MPI_Comm col_comm;
...
//			 std::cout<<"mmult Communicator generated"<< std::endl;
		 ...
		 BICGSTAB(index_type row_num , index_type col_num ,  std::string path , std::function<value_type (index_type ,  std::vector<value_type>)> init_v , std::vector<value_type> init_v_values , value_type val_1 = (value_type)0  ) 
		{
		 ...
		 BICGSTAB(index_type row_num , index_type col_num  , value_type val  )
                {
                                generate_vals( left , right , bottom , top , row_start , row_end , col_start , col_end,  col_num , row_num, comm );
        			communicator_initialization();
                       		A.allocate(row_start, row_end , col_start, col_end , &initializer_parallel::Matrix_initializer<long int,double>);
                                x.allocate(col_start , col_end , 0);
                                b.allocate( initializer_parallel::Vector_constant_initializer_matrix<index_type , value_type>(   A , col_start ,  val));
                }
...
//				std::cout << "\tProc " << myid << " has colour " << mmult_color << std::endl;
			...
			MPI_Comm_split(comm , mmult_color, myid, &mmult_comm_recv);
//			std::cout << "\tProc " << myid << " added to row communicator "  << std::endl;
			...
			int row_color = myid % proc_grid[0];
			MPI_Comm_split(comm , row_color, myid, &mmult_comm_send);
